...
" minor modifications by Horst Gutmann. You can find the original source file
" on https://github.com/ledger/vim-ledger/blob/master/syntax/ledger.vim
" vim:ts=2:sw=2:sts=2:foldmethod=marker

scriptencoding utf-8

if exists('b:current_syntax') && (b:current_syntax == "custom_ledger" || b:current_syntax == "custom_hledger")
  finish
endif
...
exe 'syn region ledgerTransaction start=/^[[:digit:]~=]/ '.
  ...
  \ 'contains=ledgerTransactionDate,ledgerTransactionConfirmed,ledgerTransactionMetadata,ledgerPosting,ledgerTransactionExpression'
syn match ledgerTransactionDate /^\d\S\+/ contained
syn match ledgerTransactionConfirmed /\s\*\s.*/ contained
...
let b:current_syntax = b:is_hledger ? 'custom_hledger' : 'custom_ledger'
